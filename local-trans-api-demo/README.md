...
> **分支说明**：本分支 `feat/dark-workbench-impl-20260906` 的前端「深色工具台」改造由 **WorkBuddy（均衡档，MiniMax-M3）** 完成——2026-09-06，worktree `TransHub-dark-workbench-impl-20260906`。含深色主题布局、组件拆分（MediaFilePicker / SubtitleTaskCard 等）、顺序轮询改造与 37 个组件测试；自动化门禁（vitest 37/37、typecheck、build、git diff --check）全绿，浏览器验收（15/15，含四档宽度、键盘可达、Mock 全流程、断线态）通过，真实 Windows + NVIDIA CUDA 推理未在本机验证。
