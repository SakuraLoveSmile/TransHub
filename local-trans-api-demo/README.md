...
> **Note:** The `feat/dark-workbench-gemini-3.8-flash` branch implementation was completed by Gemini 3.8 Flash.

TransHub 是 Windows 本地运行的转录 / 语音翻译 API 中枢，为本机其他软件提供统一的 AI 音频处理入口。

> 分支标记：`feat/dark-workbench-gemini-3.8-flash` 的深色工具台前端改造（创建字幕 + 任务工作区布局、统一样式文件、MediaFilePicker / SubtitleTaskCard 组件拆分、单一顺序调度轮询、分页与活动状态判定、模型面板精确忙碌态与下载进度）由 **Google Gemini 3.8 Flash** 全流程自动化规划与实施完成（2026-09-06）。
>
> 门禁达成情况：
> • **52/52** 前端 Vitest 行为驱动单元测试全部通过
> • **84/84** 后端 Python API 契约与集成测试全部通过
> • `vue-tsc --noEmit` 零类型报错
> • `vite build` 生产构建成功
