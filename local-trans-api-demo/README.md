...
## 深色工具台前端（feat/dark-workbench-impl）

> **实现记录**：本分支（`feat/dark-workbench-impl`，worktree：
> `TransHub-dark-workbench-impl`）的深色字幕工作台前端由 **DeepSeek V4 Flash Vision（max 推理强度）** 完成。

**实现内容**：纵向表单改为“创建字幕 + 任务工作台”双栏布局（左 340px / 右自适应，≤960px 单栏），
新增统一样式文件、`MediaFilePicker`（拖拽/校验/移除）、`SubtitleTaskCard`（折叠/复制/下载）、
紧凑服务状态条、模型管理（pending 按钮文案、真实/未知下载进度、刷新状态），
轮询改为无重叠顺序调度（有活动任务 2s / 空闲 5s）。

**完成数据（2026-09-05）**：

| 指标 | 数值 |
|---|---|
| 完成时间 | 15:43（本地时区，当日 15:01 会话开始，约 42 分钟） |
| 输出 tokens（生成） | ~229,725 |
| 输入 tokens（含缓存命中） | ~12,202,783 |
| 总 tokens | ~12,432,508 |
| 测试 | 50/50 通过（6 个测试文件） |
| typecheck / build | 通过 |
| 浏览器验收 | 1440 / 1024 / 768 / 390 无横向溢出；Mock 全流程（选文件→提交→结果→下载）通过 |

> 说明：token 数据取自 DSH 会话统计（含模型缓存命中），仅作工程记录，不构成精确计费数据。
